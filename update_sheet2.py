...
# ✅ FIXED: Credentials.from_service_account_info use किया
...
def fetch_bhavcopy_for_date(date_obj):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"HTTP {response.status_code} for date {date_obj.strftime('%d-%b-%Y')}")
        return None
    except Exception as e:
        print(f"Error fetching {date_obj.strftime('%d-%b-%Y')}: {e}")
        return None
...
for i in range(7):
    ...
    if test_date.weekday() >= 5:  # Skip Saturday(5) & Sunday(6)
        continue
    
    print(f"Trying date: {test_date.strftime('%d-%b-%Y')}...")
    data_to_insert = fetch_bhavcopy_for_date(test_date)
    
    if data_to_insert:
        fetched_date_str = test_date.strftime('%d-%b-%Y')
        print(f"Data found for {fetched_date_str}!")
        break
...
if data_to_insert:
    try:
        ...
        print(f"SUCCESS: Sheet updated for {fetched_date_str}!")
    except Exception as e:
