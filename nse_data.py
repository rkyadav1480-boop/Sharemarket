...
def load_sector_history():
    if os.path.exists(SECTOR_HISTORY_FILE):
        try:
            with open(SECTOR_HISTORY_FILE, "r") as f: return json.load(f)
        except: return {}
    return {}
...
def save_today_sectors(sector_perf):
    """Aaj ke sector results save karo — +0.5% positive, -0.5% negative"""
    today_str = date.today().strftime("%Y-%m-%d")
    history   = load_sector_history()

    # 30 din se purana data clean karo
    today_dt = date.today()
    for k in [k for k in list(history.keys())
              if (today_dt - datetime.strptime(k, "%Y-%m-%d").date()).days > 30]:
        del history[k]

    # Aaj ke sectors store karo: {sector: "positive"/"negative"/"neutral"}
    today_data = {}
    for sec, avg, count_s, gainers, losers in sector_perf:
        if avg > 0.5:
            today_data[sec] = "positive"
        elif avg < -0.5:
            today_data[sec] = "negative"
        else:
            today_data[sec] = "neutral"

    history[today_str] = today_data
    save_sector_history(history)
    print(f"💾 Sector history saved: {len(today_data)} sectors ({today_str})")
    return history

def get_sector_streaks(sector_perf, history):
    """
    Har sector ke liye consecutive positive/negative streak nikalo.
    Returns: dict { sector: (streak_days, direction) }
    """
    today_str = date.today().strftime("%Y-%m-%d")
    past_dates = sorted(
        [d for d in history.keys() if d != today_str],
        reverse=True
    )

    streaks = {}
    for sec, avg, _, _, _ in sector_perf:
        if avg > 0.5:
            today_dir = "positive"
        elif avg < -0.5:
            today_dir = "negative"
        ...
        if today_dir == "neutral":
            streaks[sec] = (0, "neutral")
            continue

        streak = 1  # aaj ka din
        for d in past_dates:
            past_dir = history.get(d, {}).get(sec, "neutral")
            if past_dir == today_dir:
                streak += 1
            else:
                break

        streaks[sec] = (streak, today_dir)

    return streaks

def make_sector_strength_bar(days, direction):
    filled = "🟩" if direction == "positive" else "🟥"
    empty  = "⬜"
    if days >= 8:   return filled * 5
    elif days >= 5: return filled * 4 + empty
    elif days >= 3: return filled * 3 + empty * 2
    elif days >= 2: return filled * 2 + empty * 3
    else:           return filled + empty * 4
...
def fetch_and_send():
    ...
    # ━━ SECTOR HISTORY SAVE + STREAK MESSAGE ━━
    sector_history = save_today_sectors(sector_perf)
    sector_streaks = get_sector_streaks(sector_perf, sector_history)

    # Sirf 2+ din streak wale sectors dikhao (1 din wale boring hain)
    strong_sectors = [
        (sec, days, direction)
        for sec, (days, direction) in sector_streaks.items()
        if days >= 2
    ]
    strong_sectors.sort(key=lambda x: -x[1])  # longest streak pehle

    if strong_sectors:
        msg_streak = "📊 <b>SECTOR MOMENTUM STREAKS</b>\n"
        msg_streak += "<i>Lagaataar positive/negative sectors:</i>\n\n"
        for sec, days, direction in strong_sectors:
            bar   = make_sector_strength_bar(days, direction)
            emoji = "📈" if direction == "positive" else "📉"
            label = "Bullish" if direction == "positive" else "Bearish"
            msg_streak += (
                f"{emoji} <b>{sec}</b> — {label}\n"
                f"⚡ {bar} (<b>{days} Din</b>)\n\n"
            )
        send_telegram(msg_streak)
